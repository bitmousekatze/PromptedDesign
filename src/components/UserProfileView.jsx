...
import BadgeTabs from './BadgeTabs.jsx';
        ...
        <div className="profile-posts-tabs" style={{
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flex: 1, minWidth: '300px' }}>
            <BadgeTabs 
              ...
              setActive={setProfilePostsTab}
              items={[
                { value: 'builds', label: 'Builds' },
                { value: 'posts', label: 'Discussion' },
                { value: 'questions', label: 'Questions' },
                ...((isOwnProfile || userPosts.some(p => p.is_repost)) ? [{ value: 'reposts', label: 'Reposts', badge: userPosts.filter(p => p.is_repost).length || undefined }] : []),
                ...(profileWorkflowsList.length > 0 ? [{ value: 'workflows', label: 'Workflows' }] : []),
                ...((profileSkillsList.length > 0 || isOwnProfile) ? [{ value: 'skills', label: 'Skills', badge: profileSkillsList.length || undefined }] : []),
                { value: 'channel', label: `${profile?.display_name || profile?.username || 'User'}'s Channel` },
                ...(ownedCommunities.length > 0 ? [{ value: 'communities', label: 'Communities' }] : [])
              ]} 
            />
          </div>

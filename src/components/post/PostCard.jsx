...
// POST CARD COMPONENT
// ============================================
const includesId = (list, id) => Array.isArray(list) && list.includes(id);

/** Skip re-render unless this card's post or per-card slice of shared state changed. */
function postCardPropsAreEqual(prev, next) {
  if (prev.post !== next.post) return false;

  const postId = prev.post.id;

  if (prev.userLikes !== next.userLikes) {
    if (includesId(prev.userLikes, postId) !== includesId(next.userLikes, postId)) return false;
  }
  if (prev.userSaves !== next.userSaves) {
    if (includesId(prev.userSaves, postId) !== includesId(next.userSaves, postId)) return false;
  }
  if (prev.pinnedPostIds !== next.pinnedPostIds) {
    if (includesId(prev.pinnedPostIds, postId) !== includesId(next.pinnedPostIds, postId)) return false;
  }
  if (prev.postCommunities !== next.postCommunities) {
    if (prev.postCommunities[postId] !== next.postCommunities[postId]) return false;
  }
  if (prev.forkedPostsMap !== next.forkedPostsMap && prev.post.forked_from_post_id) {
    const forkId = prev.post.forked_from_post_id;
    if (prev.forkedPostsMap[forkId] !== next.forkedPostsMap[forkId]) return false;
  }

  return (
    prev.categories === next.categories &&
    prev.schoolsData === next.schoolsData &&
    prev.builderRanks === next.builderRanks &&
    prev.userCommunityIds === next.userCommunityIds &&
    prev.userCommunities === next.userCommunities &&
    prev.allPosts === next.allPosts &&
    prev.communityCreatorId === next.communityCreatorId
  );
}
    ...
    >
      ...
      <div className="post-header-mobile" onClick={(e) => e.stopPropagation()}>
        <div className="post-author-section" onClick={() => { if (onUserClick) { recordProfileVisit(post.id); onUserClick(post.user_id); } }} style={{ cursor: onUserClick ? 'pointer' : 'default' }}>
          ...
          >
            {post.avatar_url ? (
              <img src={post.avatar_url} alt="" className="post-avatar-img" loading="lazy" decoding="async" />
            ) : post.avatar_emoji ? (
...
export default React.memo(PostCard, postCardPropsAreEqual);

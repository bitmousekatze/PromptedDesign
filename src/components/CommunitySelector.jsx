...
import styles from './CommunitySelector.module.css';
...
const CommunitySelector = ({ userCommunities, selectedCommunityIds = [], onSelect, preSelectedCommunityId = null, isOpen = false, onOpenChange = () => {} }) => {
  ...
  return (
    <div className={styles.selector} style={{ position: 'relative' }} ref={dropdownRef}>
      <button
        type="button"
        className={`${styles.toggle} ${selectedCommunityIds.length > 0 ? styles.selected : ''}`}
          ...
          {userCommunities.map(community => {
            ...
            return (
              ...
              >
                ...
                <span className={styles.optionName}>{community.name}</span>
                {isPreSelected && (
                  <span style={{
                    marginLeft: 'auto',
                    fontSize: '0.7rem',

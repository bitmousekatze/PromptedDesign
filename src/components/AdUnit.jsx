...
export function SidebarAd({ isAdmin = false, isPro = false }) {
  ...
  return (
    <div className={`sidebar-section ${styles.section}`}>
...
export function BottomAdBanner({ isAdmin = false, isPro = false }) {
  ...
  return (
    <div className={styles.bottomBanner} role="complementary" aria-label="Advertisement">
      <button
        className={`${styles.close} ${styles.closeBanner}`}
        aria-label="Close ad"
        onClick={() => {
          setDismissed('bottom');
          setClosed(true);

...
function renderMentionsAndUrls(text, onUserClick, keyPrefix) {
  if (!text) return null;
  const parts = [];
  let lastIndex = 0;
  let match;
...
function renderInlineMarkdown(text, onUserClick, keyPrefix) {
  if (!text) return null;
  const out = [];
  let lastIndex = 0;
  let match;
  ...
  while ((match = re.exec(text)) !== null) {
    if (match.index > lastIndex) {
      ...
      const inner = renderMentionsAndUrls(plain, onUserClick, `${keyPrefix}-p${i}`);
    ...
    const innerNodes = renderMentionsAndUrls(inner, onUserClick, `${keyPrefix}-fmt${i}`);
  ...
  if (lastIndex < text.length) {
    ...
    const inner = renderMentionsAndUrls(plain, onUserClick, `${keyPrefix}-tail`);
    if (Array.isArray(inner)) out.push(...inner);
    else if (inner != null) out.push(inner);
  }
  return out;
...
function walkDomToReact(node, onUserClick, key) {
  const children = [];
  ...
  for (let i = 0; i < kids.length; i++) {
    ...
    if (child.nodeType === 3 /* TEXT_NODE */) {
      ...
      if (!reactTag) {
        ...
        const inner = walkDomToReact(child, onUserClick, ck);
        children.push(...inner);
      } else if (reactTag === 'br') {

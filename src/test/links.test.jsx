...
describe('RichText linkify (post descriptions)', () => {
  ...
  it('does not nest anchors when stored HTML has nested <a> tags', () => {
    const { container } = render(
      <RichText text='<a href="https://example.com/outer"><div>stats <a href="https://example.com/inner">inner</a></div></a>' />
    );
    // Invalid HTML may flatten to siblings in the parser, but must never render <a> inside <a>.
    expect(container.querySelector('a a')).toBeNull();
    expect(container.textContent).toContain('inner');
  });

  it('does not autolink bare URLs inside an existing anchor', () => {
    const { container } = render(
      <RichText text='<a href="https://example.com/page">visit https://example.com/other</a>' />
    );
    const anchors = container.querySelectorAll('a');
    expect(anchors.length).toBe(1);
    expect(anchors[0].getAttribute('href')).toBe('https://example.com/page');
    expect(anchors[0].textContent).toContain('https://example.com/other');
  });
});

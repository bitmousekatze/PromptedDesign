...
import { fetchFeedPosts, fetchBuildPosts, feedPostsQueryKey, buildPostsQueryKey, filterFeedPosts } from './lib/feedPosts.js';
...
const PasswordResetModal = React.lazy(() => import('./components/PasswordResetModal.jsx'));
import CreatePostBox from './components/CreatePostBox.jsx';
const UserProfileSidebarCard = React.lazy(() => import('./components/UserProfileSidebarCard.jsx'));
const AuthModal = React.lazy(() => import('./components/AuthModal.jsx'));
const RightSidebar = React.lazy(() => import('./components/RightSidebar.jsx'));
const CreatePostModal = React.lazy(() => import('./components/post/CreatePostModal.jsx'));
import PostCard from './components/post/PostCard.jsx';
import { SkeletonBlock, SkeletonCircle, SkeletonLine, PostCardSkeleton, ListItemSkeleton, RightSidebarSkeleton } from './components/SkeletonLoader.jsx';
import PageLoader from './components/PageLoader.jsx';
const FullPostView = React.lazy(() => import('./components/post/FullPostView.jsx'));
const CreateCommunityModal = React.lazy(() => import('./components/community/CreateCommunityModal.jsx'));
const EditCommunityModal = React.lazy(() => import('./components/community/EditCommunityModal.jsx'));
const CommunitiesView = React.lazy(() => import('./components/community/CommunitiesView.jsx'));
const UserProfileView = React.lazy(() => import('./components/UserProfileView.jsx'));
const SettingsModal = React.lazy(() => import('./components/SettingsModal.jsx'));
const AccountDeletionModal = React.lazy(() => import('./components/AccountDeletionModal.jsx'));
import { deleteMyAccountNow, scheduleMyAccountDeletion, cancelMyAccountDeletion } from './lib/accountDeletion.js';
const MessagesView = React.lazy(() => import('./components/MessagesView.jsx'));
...
const WorkflowCard = React.lazy(() => import('./components/WorkflowCard.jsx'));
const CreateWorkflow = React.lazy(() => import('./components/CreateWorkflow.jsx'));
const WorkflowDetail = React.lazy(() => import('./components/WorkflowDetail.jsx'));
import { SidebarAd } from './components/AdUnit.jsx';
const MaintenanceBanner = React.lazy(() => import('./components/MaintenanceBanner.jsx'));
const DailyRewardModal = React.lazy(() => import('./components/DailyRewardModal.jsx'));
const NotificationFx = React.lazy(() => import('./components/NotificationFx.jsx'));
const ProfileChannels = React.lazy(() => import('./components/ProfileChannels.jsx'));
import BadgeSVG, { getBadgeForPoints } from './components/BadgeSVG.jsx';
import BadgeTabs from './components/BadgeTabs.jsx';
// Page-level code splitting: every page/tab is lazy-loaded so users only
// download the pages they actually visit. This cuts initial JS by ~85%.
const BuilderRanksPage  = React.lazy(() => import('./pages/BuilderRanksPage.jsx'));
const ArenaPage         = React.lazy(() => import('./pages/ArenaPage.jsx'));
const GamesPage         = React.lazy(() => import('./pages/GamesPage.jsx'));
const LearningPage      = React.lazy(() => import('./pages/LearningPage.jsx'));
const SpotlightPage     = React.lazy(() => import('./pages/SpotlightPage.jsx'));
const ProPage           = React.lazy(() => import('./pages/ProPage.jsx'));
const ZoePage           = React.lazy(() => import('./pages/ZoePage.jsx'));
const ReferralsPage     = React.lazy(() => import('./pages/ReferralsPage.jsx'));
const VideosPage        = React.lazy(() => import('./pages/VideosPage.jsx'));
const MemesPage         = React.lazy(() => import('./pages/MemesPage.jsx'));
const AchievementsPage  = React.lazy(() => import('./pages/AchievementsPage.jsx'));
const WeeklyReportPage  = React.lazy(() => import('./pages/WeeklyReportPage.jsx'));
const ReviewDraftPage   = React.lazy(() => import('./pages/ReviewDraftPage.jsx'));
const DraftsListPage    = React.lazy(() => import('./pages/DraftsListPage.jsx'));
const LiveBanner = React.lazy(() => import('./components/LiveBanner.jsx'));
import AchievementsRealtimeProvider from './components/achievements/AchievementsRealtimeProvider.jsx';
const ToolArenaRankings = React.lazy(() => import('./components/ToolArenaRankings.jsx'));
const CategoryArenaLeaders = React.lazy(() => import('./components/CategoryArenaLeaders.jsx'));
import {
  getWorkflows,
  getUserWorkflows,
  likeWorkflow,
...
const AchievementsPageWrapper = (props) => {
  ...
  return (
    <React.Suspense fallback={null}>
      <AchievementsPage {...props} addToast={addToast} />
    </React.Suspense>
  );
};
...
const LeftSidebar = ({
  ...
  const handleNavClick = (tab) => {
    ...
    // Wrap tab-switching state in startTransition so React keeps the current
    // page visible while the new lazy chunk loads (no full-app flash).
  ...
  return (
    <>
      ...
      <aside className={`left-sidebar ${isOpen ? 'open' : ''}`}>
        {/* Sidebar Header (Logo + What is Prompted?) */}
        <div className="sidebar-header">
          <div onClick={handleLogoClick} style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
            <img src="/logo-icon.svg" alt="Prompted" style={{ width: '22px', height: '22px', objectFit: 'contain' }} />
            <span className="sidebar-logo-text" style={{ fontSize: '1.25rem', marginTop: '2px' }}>Prompted</span>
          </div>
          <a
            ...
            style={{ margin: 0, fontSize: '10px', whiteSpace: 'nowrap' }}
          >What is Prompted?</a>
        </div>
        ...
        <div className="sidebar-nav-list">
          {/* Top 3-tab selector */}
          <div className="sidebar-top-tabs" style={{ display: 'flex', padding: '0 0.5rem 0.5rem', marginBottom: '0.5rem' }}>
            <BadgeTabs
              active={(activeTab === 'questions') ? 'questions' : (activeTab === 'foryou' && feedSubTab !== 'notifications' && homeContentTab === 'posts') ? 'posts' : (activeTab === 'foryou' && feedSubTab !== 'notifications' && homeContentTab === 'builds') ? 'builds' : ''}
              setActive={(val) => {
                if (val === 'builds') {
                  handleNavClick('foryou');
                  if (setHomeContentTab) setHomeContentTab('builds');
                } else if (val === 'posts') {
                  handleNavClick('foryou');
                  if (setHomeContentTab) setHomeContentTab('posts');
          ...
          <button
            ...
            onClick={() => {
              handleNavClick('foryou');
              // Optional: defaulting to builds if no subtab set? It remains unchanged for now.
            }}
            title="Home"
          >
          ...
          <button
            ...
            onClick={() => {
              ...
              if (onDailyRewardClick) onDailyRewardClick();
            }}
            title="Daily Reward"
          >
            <span className="sidebar-nav-icon"><GiftIcon size={22} /></span>
            <span className="sidebar-nav-label">Daily Reward</span>
            {dailyRewardClaimable && <span className="sidebar-nav-badge-dot" style={{ background: '#f59e0b', width: 8, height: 8, borderRadius: '50%', marginLeft: 8 }} />}
          </button>
...
const TermsPage = ({ onBack, backLabel = "Back to Settings" }) => {
  return (
...
const PrivacyPage = ({ onBack, backLabel = "Back to Settings" }) => {
  return (
    <div className="legal-page">
      <div className="legal-page-header">
        <button className="legal-back-btn" onClick={onBack}>
...
const SupportPage = ({ onBack, backLabel = "Back to Settings" }) => {
  return (
    <div className="legal-page">
      <div className="legal-page-header">
        <button className="legal-back-btn" onClick={onBack}>
...
const CopyrightPage = ({ onBack, backLabel = "Back to Settings" }) => {
  return (
    <div className="legal-page">
      <div className="legal-page-header">
        <button className="legal-back-btn" onClick={onBack}>
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 600 }}>
        ...
        <div className="modal-body">
          ...
          {loading ? (
            <div style={{ textAlign: 'center', padding: '2rem' }}><PageLoader size={20} /></div>
          ) : items.length === 0 ? (
...
const SearchPage = ({ categories, onOpenFullPost, onUserClick, onCategoryClick, onToolClick, onAuthRequired }) => {
  ...
  return (
    <div className="search-page">
      ...
      {/* Results */}
      {loading ? (
        <div className="search-loading"><PageLoader size={24} text="" /></div>
...
function VibeShareAppInner() {
  ...
  const setPosts = useCallback((updater) => {
    queryClient.setQueryData(feedPostsQueryKey(user?.id), (old = []) =>
      typeof updater === 'function' ? updater(old) : updater
    );
  }, [queryClient, user?.id]);
  const loadPosts = useCallback(() => {
    queryClient.invalidateQueries({ queryKey: feedPostsQueryKey(user?.id) });
  }, [queryClient, user?.id]);
  ...
  const appDashboardVisible = !!user || landingDismissed;
  const buildsFeedEnabled = appDashboardVisible
    && activeTab === 'foryou'
    && homeContentTab === 'builds'
    && feedSubTab !== 'notifications';
  const buildsFeedQuery = useQuery({
    queryKey: buildPostsQueryKey(),
    queryFn: fetchBuildPosts,
    staleTime: 1000 * 60,
    enabled: buildsFeedEnabled,
  });
  const buildPosts = buildsFeedQuery.data ?? [];
  const buildsLoading = buildsFeedQuery.isLoading;
  const setBuildPosts = useCallback((updater) => {
    queryClient.setQueryData(buildPostsQueryKey(), (old = []) =>
      typeof updater === 'function' ? updater(old) : updater
    );
  }, [queryClient]);
  const loadBuilds = useCallback(() => {
    queryClient.invalidateQueries({ queryKey: buildPostsQueryKey() });
  }, [queryClient]);
  ...
  const setSelectedFullPost = (post) => {
    ...
    if (post && post.id) {
      ...
      const image = (post.images && post.images.length > 0) ? post.images[0] : OG_IMAGE_URL;
      ...
      document.querySelector('meta[property="og:image"]')?.setAttribute('content', OG_IMAGE_URL);
      ...
      document.querySelector('meta[name="twitter:image"]')?.setAttribute('content', OG_IMAGE_URL);
    }
  };
  ...
  // Critical path: feed + data needed for first paint. Everything else is
  // deferred so we don't compete with loadPosts for browser connections.
  useEffect(() => {
    loadAiTools();
    loadCategories();
    loadUserSchoolIdMap();
  ...
  // Directory / leaderboard data — nice-to-have on home, not blocking the feed.
  useEffect(() => {
    let cancelled = false;
    const run = () => {
      if (!cancelled) {
  ...
  const filterRemovedPosts = filterFeedPosts;
  ...
  const loadForkedPostOriginals = async (feedPosts) => {
    ...
    const { data: originals } = await supabase
      .from('posts')
      .select('*, profiles:user_id (id, username, display_name, avatar_emoji, avatar_url, name_color, builder_points), original_post:posts!posts_forked_from_post_id_fkey(id, title, description, prompt, prompt_steps, images, videos, ai_tool, tool_ids, created_at, user:user_id(id, username, display_name, avatar_url, avatar_emoji))')
      .in('id', uniqueIds);
  ...
  const feedEnrichedAtRef = useRef(0);
  useEffect(() => {
    if (feedFetching || !feedDataUpdatedAt) return;
    if (feedDataUpdatedAt === feedEnrichedAtRef.current) return;
    feedEnrichedAtRef.current = feedDataUpdatedAt;
    loadPostCommunities();
    if (posts.length > 0) {
      loadForkedPostOriginals(posts);
    }
  }, [feedFetching, feedDataUpdatedAt, posts]);
  ...
  return (
    <AuthContext.Provider value={{ user, profile, onboardingCompleted, userSchoolIdMap, savedPromptIds: userSavedPrompts, toggleSavePrompt: handleSavePrompt }}>
        ...
        ) : (!user && ['privacy', 'terms', 'about', 'copyright', 'support'].includes(activeTab)) ? (
          <div className="public-legal-page" style={{ background: 'var(--bg-primary)', minHeight: '100vh', overflowY: 'auto' }}>
            <header style={{ padding: '24px 40px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center' }}>
              <a href="/" onClick={(e) => { e.preventDefault(); window.location.href = '/'; }} style={{ color: '#f5ebe0', textDecoration: 'none', fontWeight: '900', fontSize: '24px', display: 'flex', alignItems: 'center', gap: '12px', fontFamily: "'Fraunces', Georgia, serif", WebkitTextStroke: '0.6px #1a1a1a', paintOrder: 'stroke fill', textShadow: '0 1px 4px rgba(0, 0, 0, 0.55)', letterSpacing: '-0.5px' }}>
                <img src="/logo-icon.svg" alt="Prompted" style={{ width: '28px', height: '28px' }} />
                Prompted
              </a>
            </header>
            <main style={{ maxWidth: '800px', margin: '0 auto', padding: '60px 20px', width: '100%' }}>
              {activeTab === 'privacy' && <PrivacyPage onBack={() => { setActiveTab('foryou'); window.history.pushState({}, '', '/'); }} backLabel="Back to Home" />}
              {activeTab === 'terms' && <TermsPage onBack={() => { setActiveTab('foryou'); window.history.pushState({}, '', '/'); }} backLabel="Back to Home" />}
              {activeTab === 'about' && <AboutPage onBack={() => { setActiveTab('foryou'); window.history.pushState({}, '', '/'); }} backLabel="Back to Home" />}
              {activeTab === 'support' && <SupportPage onBack={() => { setActiveTab('foryou'); window.history.pushState({}, '', '/'); }} backLabel="Back to Home" />}
              {activeTab === 'copyright' && <CopyrightPage onBack={() => { setActiveTab('foryou'); window.history.pushState({}, '', '/'); }} backLabel="Back to Home" />}
            </main>
          </div>
            ...
            <LandingPage
              ...
              onFooterLink={(target) => {
                if (target === 'about' || target === 'privacy' || target === 'terms') {
                  setActiveTab(target);
                  const pathMap = { about: '/about', privacy: '/privacypolicy', terms: '/termsandconditions' };
                  if (pathMap[target]) window.history.pushState({}, '', pathMap[target]);
        ...
        <div className={`app-container app-with-sidebar`}>
          ...
          <aside className={`right-drawer ${rightSidebarOpen ? 'open' : ''}`}>
            ...
            <div className="right-drawer-ofday">
              <React.Suspense fallback={<RightSidebarSkeleton />}>
              <RightSidebar
                ...
                {activeTab === 'foryou' && !viewingUserId && (
                  <div className="for-you-tab">
                    ...
                    <div className="feed-tab-switcher feed-tab-switcher-scroll">
                      ...
                      />
                      <button
                        className="mobile-feed-menu-btn"
                    ...
                    {(feedSubTab === 'foryou' || feedSubTab === 'top' || feedSubTab === 'unliked' || feedSubTab === 'random') && (
                      <div className="feed-container">
                        ...
                        {(homeContentTab === 'builds' ? buildsLoading : loading) ? (
                          <div>
                                ...
                                >
                                  ...
                                  <div style={{ overflowY: 'auto', padding: '0.5rem' }}>
                                    {approvedClaimsLoading ? (
                                      <div style={{ padding: '1.5rem', textAlign: 'center' }}><PageLoader size={16} /></div>
                  ...
                  <div className="explore-tab">
                    ...
                    {schoolLeaderboardView ? (
                      ...
                      <div className="school-page">
                        ...
                        {schoolsLoading ? (
                          <div className="loading-state"><PageLoader size={20} text="Loading school..." /></div>
                        ) : schoolDetails ? (
                          <>
                            {/* Hero Section */}
                            ...
                            {viewingSchoolTab === 'creators' ? (
                              ...
                              <div>
                                {schoolTrendingLoading ? (
                                  <div className="loading-state"><PageLoader size={20} text="" /></div>
                      ...
                      <div style={{ padding: '0' }}>
                        ...
                        <div className="category-view-posts">
                          {toolSearchQuery.trim() ? (
                            toolSearchLoading ? (
                              <div className="loading-state"><PageLoader size={20} text={`Searching ${viewingToolName} ${isToolQuestionsTab ? 'questions' : 'posts'}...`} /></div>
                    ...
                    {exploreSearchActive ? (
                      <div className="explore-search-results-container">
                        ...
                        {exploreSearchLoading ? (
                          <div className="loading-state"><PageLoader size={20} text="Searching..." /></div>
                        ) : (
                          <>
                    ...
                    <div className="explore-section">
                      ...
                      {exploreRandomPosts.length > 0 && (
                        <div style={{ display: 'flex', justifyContent: 'center', padding: '1.5rem 0' }}>
                          ...
                          >
                            {loadingMoreExplorePosts ? <PageLoader size={14} text="" /> : 'Load More Posts'}
                          </button>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="profile-posts-section">
                      <div className="profile-posts-tabs" style={{
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem', flex: 1, minWidth: '300px' }}>
                          <BadgeTabs 
                            active={myProfilePostsTab} 
                            setActive={setMyProfilePostsTab}
                            items={[
                              { value: 'builds', label: 'Builds' },
                              { value: 'posts', label: 'Discussion' },
                              { value: 'questions', label: 'Questions' },
                              { value: 'reposts', label: 'Reposts', badge: myProfilePosts.filter(p => p.is_repost).length || undefined },
                              ...(myProfileWorkflows.length > 0 ? [{ value: 'workflows', label: 'Workflows' }] : []),
                              { value: 'channel', label: `${profile?.display_name || profile?.username || 'User'}'s Channel` },
                              ...(myProfileOwnedCommunities.length > 0 ? [{ value: 'communities', label: 'Communities' }] : [])
                            ]} 
                          />
                        </div>
          ...
          <nav className="bottom-nav">
            <button
              ...
              onClick={() => { setActiveTab('foryou'); setSearchQuery(''); setActiveCommunity(null); setShowSearchPage(false); clearExploreSearch(); window.history.replaceState({}, '', '/'); }}
            >
            ...
            <button
              ...
              onClick={() => { setActiveTab('explore'); setShowSearchPage(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
            >
            ...
            <button
              ...
              onClick={() => { setActiveTab('ranks'); setSearchQuery(''); setShowSearchPage(false); clearExploreSearch(); window.history.replaceState({}, '', '/ranks'); }}
            >
              ...
              <div className="school-members-modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="school-members-modal-body">
                  {schoolMembersLoading ? (
                    <div style={{ padding: '2rem', textAlign: 'center' }}><PageLoader size={24} text="Loading members..." /></div>
          ...
          {(() => {
            const dashboardTabs = [
              { key: 'builds', label: 'Builds', renderIcon: () => <AnimatedIcon name="home" size={20} />, onClick: () => { setActiveTab('foryou'); setHomeContentTab('builds'); window.history.pushState({}, '', '/'); } },
              { key: 'posts', label: 'Discussion', renderIcon: () => <AnimatedIcon name="messages" size={20} />, onClick: () => { setActiveTab('foryou'); setHomeContentTab('posts'); window.history.pushState({}, '', '/'); } },
              { key: 'questions', label: 'Questions', renderIcon: () => <AnimatedIcon name="questions" size={20} />, onClick: () => { setActiveTab('questions'); window.history.pushState({}, '', '/questions'); } }
            ];
            if (canSeePro(profile)) {
              dashboardTabs.push({
                key: 'pro',
                label: 'Pro',
                renderIcon: () => (
                ...
                onClick: () => { setActiveTab('pro'); window.history.pushState({}, '', '/pro'); }
              });
            }
            const activeDashboardTabKey = (activeTab === 'foryou' && homeContentTab === 'builds') ? 'builds' :
                                          (activeTab === 'foryou' && homeContentTab === 'posts') ? 'posts' :
                                          activeTab === 'questions' ? 'questions' :
                                          activeTab === 'pro' ? 'pro' : null;
            const activeDashboardIndex = dashboardTabs.findIndex(t => t.key === activeDashboardTabKey);
              ...
              <nav className="dashboard-bottom-nav">
                {activeDashboardIndex !== -1 && (
                  <div
                    ...
                    style={{
                      width: `calc((100% - 12px) / ${dashboardTabs.length})`,
                      transform: `translateX(calc(100% * ${activeDashboardIndex}))`
                    }}
                  />
                )}
                {dashboardTabs.map((tab) => (
                  <button
                    key={tab.key}
                    className={`dashboard-bottom-nav-item ${activeDashboardTabKey === tab.key ? 'active' : ''}`}
                    onClick={tab.onClick}
                    style={{ zIndex: 1 }}
                  >
                    {tab.renderIcon()}

...
const UploadGameModal = React.lazy(() => import('../components/games/UploadGameModal.jsx'));
...
function GamesBrowse({ onOpen, currentUser, onRequireAuth }) {
  ...
  return (
    <>
      ...
      {loading ? <div style={gridStyle}>{[1,2,3,4,5,6].map(i => <SkeletonBlock key={i} height={220} />)}</div>
        : sorted.length === 0 ? (
...
function GamesLibrary({ currentUser, onOpen, onRequireAuth }) {
  ...
  if (rows === null) return <div style={gridStyle}>{[1,2,3,4].map(i => <SkeletonBlock key={i} height={220} />)}</div>;
  if (rows.length === 0) {
    return (
      <div style={emptyStyle}>
...
function GameDetail({ slug, currentUser, onRequireAuth, onBack, onUserClick }) {
  ...
  if (loading) return <div><SkeletonBlock height={500} style={{ borderRadius: 12, marginBottom: 16 }} /></div>;
  if (!game) return (
    <div>
        ...
        <div>
          ...
          >
            {!externalUrl && !baseHref ? (
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'100%' }}><PageLoader size={24} text="Loading game..." /></div>
            ) : (
              <iframe
                ref={iframeRef}
                srcDoc={srcDoc}
...
function LeaderboardRail({ gameId, refreshKey, currentUser, onUserClick }) {
  ...
  return (
    <div style={{ marginTop: 22 }}>
      ...
      {rows === null ? (
        <div style={{ marginTop: 10 }}><PageLoader size={16} text="" /></div>
      ) : rows.length === 0 ? (

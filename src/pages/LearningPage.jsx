...
import CafeteriaChat from '../components/CafeteriaChat.jsx';
...
export default function LearningPage({ currentUser, profile, onRequireAuth, addToast }) {
  ...
  if (loading) {
    return <div style={{ maxWidth: 1000, margin: '0 auto', padding: '40px 16px', color: C.muted }}>Loading Learning…</div>;
  }
    ...
    <section style={{ marginTop: 44 }}>
      ...
      <div className="lrn-lb-board">
        {error ? (
          ...
          <div className="lrn-lb-msg">Loading…</div>
        ) : rows.length === 0 ? (
  ...
  return (
    <div className="lrn-modal-wrap" onMouseDown={(e) => { if (e.target === e.currentTarget) close(); }}>
      <div className="lrn-modal" role="dialog" aria-modal="true">
        ...
        {!attempts ? <div style={{ color: C.muted }}>Loading…</div> : (
          <>
            {stage === 'overview' && (
...
function Quiz({ project, track, phase, onDone, toast }) {
  ...
  if (!questions) return <div style={{ color: C.muted }}>Loading quiz…</div>;
...
function SubmissionView({ submissionId, tracks, projects, currentUser, onRequireAuth, toast, onBack }) {
  ...
  if (loading) return <div style={{ color: C.muted }}>Loading…</div>;
  ...
  return (
    <div className="lrn-modal-wrap" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="lrn-modal" role="dialog" aria-modal="true">
        ...
        <div style={{ marginTop: 12 }}>
          {!items ? <div style={{ color: C.muted }}>Loading…</div> : items.length === 0 ? (
  ...
  return (
    <div className="lrn-modal-wrap" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="lrn-modal" role="dialog" aria-modal="true">
        ...
        {items === null ? <div style={{ color: C.muted }}>Loading…</div>
